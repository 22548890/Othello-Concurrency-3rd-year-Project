...
void run_worker(int rank) {
	...
	int my_colour;
	// Broadcast colour
	MPI_Bcast(&my_colour, 1, MPI_INT, 0, MPI_COMM_WORLD);//not sure if src should be rank
	// Broadcast running
	MPI_Bcast(&running, 1, MPI_INT, 0, MPI_COMM_WORLD);
	...
	while (running == 1) {
		...
		//loc=minimax strategy :giving colour and best val
		//store loc and best val in array
		//MPI_Gather
		// Generate move        
